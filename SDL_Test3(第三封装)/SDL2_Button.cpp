...
WaterBox::SDL2_Button::SDL2_Button(SDL2_Material *matBefor, SDL2_Material *matAfter)
{
	...
	m_MaterialAfter->setPosition(m_Position);
	m_MaterialBefore->setPosition(m_Position);
	m_Size = matBefor->getSize();
	m_Callback = nullptr;
...
int WaterBox::SDL2_Button::show()
{
	if (SDL2_Material::TYPE_PARENT == m_MaterialAfter->getType())
	{
		if (m_Flag)
		{
			SDL2_Draw::drawRectangle(m_Position, m_Size, Math::vec4(100, 100, 100, 180));
		}
		else
		{
			SDL2_Draw::drawRectangle(m_Position, m_Size, Math::vec4(180, 180, 180, 180));
		}
	}
	else
	{
